...
async def fetch_html_data(session, url, page_number):
...
async def get_student_list(html_code):
...
async def upload_student(session, document_id, student_data):
    databases = Databases(client)
    databases.create_document("telugu", "inter_1st_year_2022", document_id, student_data)
    return 0

async def process_student(session, i):
    html_code = await fetch_html_data(session, url, str(i))
    student_list = await get_student_list(html_code)
    for s in student_list:
        student_data = {"student_name": s["Candidate's Name"], "student_roll_no": s["Roll No"]}
        await upload_student(session, str(s["Roll No"]), student_data)
...
async def main():
    ...
    client.set_project("telinter")
    client.set_key(my_secret)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(1101, 22000):
            tasks.append(process_student(session, i))
